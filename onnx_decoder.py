...
import numpy
import torch
from dalle import DalleBartDecoderS0, DalleBartDecoderS1, DalleBartDecoderS2, DalleBartDecoder, TextTokenizer
...
if __name__ == '__main__':
    ...
    if decoder_s_id == 0:
        decoder_s = DalleBartDecoderS0(decoder)
    elif decoder_s_id == 1:
        decoder_s = DalleBartDecoderS1(decoder)
    elif decoder_s_id == 2:
        decoder_s = DalleBartDecoderS2(decoder)
    else:
        raise ValueError
    ...
    if decoder_s_id == 0:
        ...
        raise ValueError
    torch.onnx.export(decoder_s, inputs, f'onnx/decoder{decoder_s_id}/decoder{decoder_s_id}.onnx', output_names=["image_tokens", "attention_state"], verbose=True, opset_version=15, export_params=True, do_constant_folding=True)
