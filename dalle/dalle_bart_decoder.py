...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Self Attention
        self_attn_mask = (self.token_indices < token_index + 1).long()
...
class DalleBartDecoderF(nn.Module):
    def __init__(
        self,
    ...
    def forward(
        ...
        for i in range(self.layer_count):
            self.layers[i] = self.layers[i].to(torch.float32)
            ...
            self.layers[i] = self.layers[i].to(torch.float16)
...
class DalleBartDecoderS0(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        super().__init__()
        self.layer_count = model.layer_count
        self.embed_count = model.embed_count
        self.image_vocab_count = model.image_vocab_count
        self.embed_tokens = model.embed_tokens
        self.embed_positions = model.embed_positions
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(0, 8)
        ])
        self.layernorm_embedding = model.layernorm_embedding
        self.token_indices = model.token_indices
...
class DalleBartDecoderS1(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        super().__init__()
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(8, 16)
        ])
    ...
    def forward(
        ...
        for i in range(8, 16):
            decoder_state, attention_state[i-8] = self.layers[i-8].forward(
                decoder_state,
                encoder_state,
                attention_state[i-8],
                attention_mask,
...
class DalleBartDecoderS2(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        ...
        self.embed_positions = model.embed_positions
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(16, 24)
        ])
    ...
    def forward(
        ...
        for i in range(16, 24):
            decoder_state, attention_state[i-16] = self.layers[i-16].forward(
                decoder_state,
                encoder_state,
                attention_state[i-16],
                attention_mask,
                token_index
            )
        decoder_state = self.final_ln(decoder_state)
...
class DalleBartDecoderS0v2(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        ...
        self.embed_positions = model.embed_positions
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(0, 6)
        ])
        self.layernorm_embedding = model.layernorm_embedding
        self.token_indices = model.token_indices
...
class DalleBartDecoderS1v2(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        super().__init__()
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(6, 12)
        ])
    ...
    def forward(
        ...
        for i in range(6, 12):
            decoder_state, attention_state[i-6] = self.layers[i-6].forward(
                decoder_state,
                encoder_state,
                attention_state[i-6],
                attention_mask,
...
class DalleBartDecoderS2v2(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        super().__init__()
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(12, 18)
        ])
    ...
    def forward(
        ...
        for i in range(12, 18):
            decoder_state, attention_state[i-12] = self.layers[i-12].forward(
                decoder_state,
                encoder_state,
                attention_state[i-12],
                attention_mask,
                token_index
            )
        return decoder_state, attention_state
...
class DalleBartDecoderS3v2(nn.Module):
    def __init__(
        self,
        model: DalleBartDecoder
    ):
        ...
        self.embed_positions = model.embed_positions
        self.layers: List[DecoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(18, 24)
        ])
        self.final_ln = model.final_ln
        self.lm_head = model.lm_head
        self.token_indices = model.token_indices
    ...
    def forward(
        ...
        for i in range(18, 24):
            decoder_state, attention_state[i-18] = self.layers[i-18].forward(
                decoder_state,
                encoder_state,
                attention_state[i-18],
                attention_mask,
                token_index
            )
        decoder_state = self.final_ln(decoder_state)
