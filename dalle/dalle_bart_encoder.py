...
class DalleBartEncoderS0(nn.Module):
    def __init__(
        self,
        model: DalleBartEncoder
    ):
        super().__init__()
        self.text_vocab_count = model.text_vocab_count
        self.embed_tokens = model.embed_tokens
        self.embed_positions = model.embed_positions
        self.layers: List[EncoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(0, 12)
        ])
        self.layernorm_embedding = model.layernorm_embedding
        self.pose_tokens = model.pose_tokens
...
class DalleBartEncoderS1(nn.Module):
    def __init__(
        self,
        model: DalleBartEncoder
    ):
        super().__init__()
        self.layers: List[EncoderLayer] = nn.ModuleList([
            model.layers[i] for i in range(12, 24)
        ])
        self.final_ln = model.final_ln

    def forward(self, encoder_state: FloatTensor, text_tokens: LongTensor) -> FloatTensor:
        attention_mask = text_tokens.not_equal(1).float()
        for layer in self.layers:
            encoder_state = layer.forward(encoder_state, attention_mask)
        encoder_state = self.final_ln.forward(encoder_state)
